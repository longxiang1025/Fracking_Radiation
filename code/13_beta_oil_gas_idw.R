...
hpbl<-hpbl[[338:469]]
p_hpbl<-as.data.frame(raster::extract(hpbl,radnet))
p_hpbl[,"city_state"]=as.factor(radnet@data[,"city_state"])
p_hpbl<-gather(p_hpbl,time,hpbl,-"city_state")
...
p_hpbl<-p_hpbl[!duplicated(p_hpbl[,c("city_state","Year","Month")]),]
p_hpbl<-p_hpbl[,c("city_state","hpbl","Year","Month")]
...
beta_wind_hpbl<-left_join(beta_wind,p_hpbl,by=c("YEAR"="Year","MONTH"="Month","city_state"="city_state"))
...
beta_wind_hpbl<-beta_wind_hpbl%>%filter(YEAR>2006&YEAR<2018)
rad_all<-left_join(beta_wind_hpbl,gas_prod)
rad_all<-left_join(rad_all,gas_fst_prod)
rad_all<-left_join(rad_all,gas_snd_prod)
rad_all<-left_join(rad_all,gas_prod_wind)
rad_all<-left_join(rad_all,gas_fst_prod_wind)
rad_all<-left_join(rad_all,gas_snd_prod_wind)
rad_all<-left_join(rad_all,oil_prod)
rad_all<-left_join(rad_all,oil_fst_prod)
rad_all<-left_join(rad_all,oil_snd_prod)
rad_all<-left_join(rad_all,oil_prod_wind)
rad_all<-left_join(rad_all,oil_fst_prod_wind)
rad_all<-left_join(rad_all,oil_snd_prod_wind)
...
rad_all<-left_join(rad_all,density_snd)
rad_all<-left_join(rad_all,density_snd_wind)

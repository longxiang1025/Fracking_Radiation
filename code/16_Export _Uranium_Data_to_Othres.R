...
#Uranium-238 is annual while the frequency of beta is 3 days. So, in this dataset, we need to aggregate the drill 
# Besides the drilling/production data, weather data are also collected per month. In this cases, at first, all 
#data except for the Uranium-238 data is still organized by month. Before joining the annual Uranium-238 data, we just 
...
for(r in c(25000,50000,75000,100000)){
  ...
  rad_beta<-rad_beta%>%filter(YEAR<2015 & YEAR>2007)
  ...
  WHERE \"Monitor_ID\" = \"ID\" AND (\"Date Local\" BETWEEN '01/01/2008' AND '12/31/2014')
  GROUP BY \"city\"
  "
  ...
  city_well_relation<-city_well_relation%>%filter(`Last Prod Date`>2007)
  prod_db <- tbl(con, "Well_Production_Table")
  prod_db<-prod_db%>%filter(API%in%city_well_relation$`API/UWI`&Prod_Year>2007&Prod_Year<2015)
  print(Sys.time())
  ...
  wind_hpbl<-beta_wind_hpbl%>%group_by(city_state)%>%summarise(vel=mean(vel),
                                                               hpbl=mean(hpbl))
